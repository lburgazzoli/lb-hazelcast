...
public class ClasspathJsonConfig extends Config {
    private static final Logger LOGGER = LoggerFactory.getLogger(ClasspathJsonConfig.class);

    public ClasspathJsonConfig(String resource) {
        this(resource, System.getProperties());
    }

    public ClasspathJsonConfig(String resource, Properties properties) {
        this(Thread.currentThread().getContextClassLoader(), resource, properties);
    }

    public ClasspathJsonConfig(ClassLoader classLoader, String resource) {
        this(classLoader, resource, System.getProperties());
    }

    public ClasspathJsonConfig(ClassLoader classLoader, String resource, Properties properties) {
        if (classLoader == null) {
        ...
        new JsonConfigBuilder(in).build(this);
    }
}
